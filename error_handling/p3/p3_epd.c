...
void notified(microkit_channel ch) {
    ...
    switch (ch) {
        case SCHEDULER_CH_ID: {
            // sddf_dprintf("IN EPD 3\n");
            /* Init */
            /* Assign error handling functions after the PPD, SPD, APD have run */
            if (!init_finished) {
                microkit_dbg_puts("P3 following up Initialising!\n");
            ...
            /* recovery function and yield */
                ...
                if (PPD_STATUS->handle_error_fn != 0) {
                    microkit_pd_restart(APD_ID, (seL4_Word) P_STATE->apd_error_hdl);
                }
                return;
             } else {
            /* No aPD - do nothing */
