...
#define EPD_TIMESLICE 1010 * NS_IN_MS

#define POST_RECOVERY_PADDING 1010 * NS_IN_MS

#define NUM_PARTITIONS 3

/* Partition specific */
#define P1_LEN 1020 * NS_IN_MS
#define P2_LEN 1020 * NS_IN_MS
#define P3_LEN 1020 * NS_IN_MS
...
uint64_t get_prev_normal_partition_idx(void) {
    int curr_head = get_curr_partition() - 1;
...
void notified(microkit_channel ch)
{
    ...
    case TIMER_CH_ID:
        ...
        // sddf_dprintf("Partition to run: %d\n", partition_to_run);
        // sddf_dprintf("Is callback? %d\n", is_callback);
        ...
        if (!is_callback) {
            if (partition_to_run % 2 != 0) {
                ...
                if (first_run) {
                    ...
                    int prev_normal = get_prev_normal_partition_idx();
                    // sddf_dprintf("Previous normal: %d\n", prev_normal);
                    if ((partition_state_list[get_prev_normal_partition_idx()]->recovering_pd & RECOVERY_MASK) == PPD_RECOVERING) {
                        ...
                        is_callback = 1;
                        callback_partition = partition_to_run;
                        return;
                    } else {
                        sddf_dprintf("Padding\n");
                        ...
                        is_callback = 1;
                        callback_partition = partition_to_run;
                        // sddf_dprintf("Callback partition: %ld\n", callback_partition);
                        return;
                    }                
                }
            }
